...
def compute_model_output(model, inputs, src_emb, tar_emb, delta):
    '''
    ...
    if model.separate:
        ...
        all_outputs = model(inputs, delta = delta, src_emb = src_emb, tar_emb = tar_emb)

    return all_outputs

# def predict(audio, src_emb, tar_emb, delta, model):
#     '''
...
#             # Predict
#             for key, curr_targets in compute_model_output(model, curr_input, src_emb, tar_emb, delta).items():
#                 noise = curr_targets.squeeze(0)
...
#                 curr_input = curr_input.squeeze(0).cpu().numpy()
#                 outputs[key][:,target_start_pos:target_start_pos+model.shapes["output_frames"]] = noise + curr_input[:, model.shapes["output_start_frame"]:model.shapes["output_end_frame"]]
...
def predict_wav(args, audio_path, model):
    '''
    ...
    with torch.no_grad():
        for target_start_pos in range(0, target_outputs, model.shapes["output_frames"]):
            ...
            for key, curr_targets in compute_model_output(model, curr_input, src_emb, tar_emb, delta).items():
                ...
                noise = noise - noise.mean(dim = 1).unsqueeze(1).repeat(1,noise.shape[1])
                noise = noise.cpu()
                curr_input = curr_input.squeeze(0)
                clean = curr_input[:,model.shapes["output_start_frame"]:model.shapes["output_end_frame"]]
                ...
                noisenewlevel = noisenewlevel.numpy()
                clean = clean.numpy()
                outputs[key][:,target_start_pos:target_start_pos+model.shapes["output_frames"]] = clean + noisenewlevel

    # Crop to expected length (since we padded to handle the frame shift)
    for key in outputs.keys():
        m = np.max(outputs[key][:])
        outputs[key][:] = outputs[key][:] / m
    outputs = {key : outputs[key][:,:expected_outputs] for key in outputs.keys()}
