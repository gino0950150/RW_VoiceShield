...
def main(args):
    ...
    delta = 20
    with open("/homes/jinyu/attack_vc/data/sub_test_pairs.txt", "r") as f:
        lines = f.readlines()
    lines = [line.rstrip().split() for line in lines] 
    i = 0
    root = os.path.join(args.exp_dir, "result_sub")
    L_attack_fail = []
    L_preserve_success = []
    ...
    for src_path, tar_path, adv_path in tqdm(lines):
        ...
        sf.write(os.path.join(dir, "0.wav"), adv_wav.squeeze(), args.sr) # adv_input
        sf.write(os.path.join(dir, "base.wav"), tar.squeeze(), args.sr)
        # sf.write(os.path.join(dir, "adv_spk.wav"), adv, args.sr)
        # sf.write(os.path.join(dir, "content.wav"), src, args.sr)
        before = utils.vc_infer(src_path, os.path.join(dir, f'base.wav'), vc_smodel, cmodel, net_g, hps, vc_utils)
        after = utils.vc_infer(src_path, os.path.join(dir, f'0.wav'), vc_smodel, cmodel, net_g, hps, vc_utils)
        sf.write(os.path.join(dir, '1.wav'), before, args.sr) #before
        sf.write(os.path.join(dir, '2.wav'), after, args.sr) #after
        # sf.write(os.path.join(dir, 'noise.wav'), adv_wav.squeeze() - tar.squeeze()/m, args.sr)
        
        attack_fail = utils.sv(os.path.join(dir, '2.wav'), os.path.join(dir, "base.wav"), emb_model)
        preserve_success = utils.sv(os.path.join(dir, '0.wav'), os.path.join(dir, 'base.wav'), emb_model)
