...
def evaluate(val_dataloader,model,smodel, delta, criterion):
    model.eval()
    smodel.eval()
    losses = []
    s = 0
    for example_num, (x, src_emb, tar_emb, src_spk, tar_spk) in tqdm(enumerate(val_dataloader)):
        x = x.cuda()
        noise = model(x, delta = None, src_emb = src_emb, tar_emb = tar_emb)["adv_noise"].squeeze()
        noise = noise - noise.mean(dim = 1).unsqueeze(1).repeat(1,noise.shape[1])
        x = x.squeeze()
        clean = x[:,model.shapes["output_start_frame"]:model.shapes["output_end_frame"]]
        rmsclean = (clean**2).mean(dim = 1)**0.5
        rmsnoise = (noise**2).mean(dim = 1)**0.5
        rmsclean =rmsclean.unsqueeze(1).repeat(1,noise.shape[1])
        rmsnoise = rmsnoise.unsqueeze(1).repeat(1,noise.shape[1])
        noisescalar = rmsclean / (10**(delta/20)) / (rmsnoise+EPS)
        noisenewlevel = noise * noisescalar
        noisyspeech = clean + noisenewlevel
        m = torch.abs(noisyspeech).max(dim = 1)[0].unsqueeze(1).repeat(1,noise.shape[1])
        x_adv = torch.div(noisyspeech, m)
        ...
        adv_emb = adv_emb.cuda()
        loss = criterion(adv_emb, tar_emb) + 0.1*criterion(adv_emb, src_emb)
...
#         adv_emb = smodel.mel_embed_utterance(mel_x)
#         adv_emb = adv_emb.cuda()
#         loss = criterion(adv_emb, tar_emb, torch.ones(adv_emb.shape[0]).cuda()) + 0.1*criterion(adv_emb, src_emb, -1*torch.ones(adv_emb.shape[0]).cuda())
#         losses.append(loss.item())
#         s += 1
#         if s >= 500:
#             break
...
def main(args):
    ...
    smodel = SpeakerEncoder('/homes/jinyu/attack_vc/FreeVC/speaker_encoder/ckpt/pretrained_bak_5805000.pt').cuda()
    smodel_vc = SpeakerEncoder('/homes/jinyu/attack_vc/FreeVC/speaker_encoder/ckpt/pretrained_bak_5805000.pt').cuda()
    ...
    train_data = VCTKDataset("/homes/jinyu/attack_vc/data/train.txt", args.instruments, args.sr, args.channels, model.shapes, True, smodel)
    val_data = VCTKDataset("/homes/jinyu/attack_vc/data/val.txt", args.instruments, args.sr, args.channels, model.shapes, True, smodel)
    ...
    val_dataloader = torch.utils.data.DataLoader(val_data, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, worker_init_fn=utils.worker_init_fn)
    ...
    while True:
        ...
        for example_num, (x, src_emb, tar_emb, src_spk, tar_spk) in tqdm(enumerate(train_dataloader)):
            ...
            m = torch.abs(noisyspeech).max(dim = 1)[0].unsqueeze(1).repeat(1,noise.shape[1])
            x_adv = torch.div(noisyspeech, m)
            clean = torch.div(clean, m)
            noise = torch.div(noisenewlevel, m)
            ...
            if state["step"] % 10000 == 0:
                ...
                sf.write(os.path.join(dir, 'clean.wav'), clean[0,:].detach().cpu().numpy(), args.sr)
                c = random.choice(train_data.audiolist)
                before = utils.vc_infer(c, os.path.join(dir, f'ori_input.wav'), smodel_vc, cmodel, net_g, hps, vc_utils)
                after = utils.vc_infer(c, os.path.join(dir, 'adv_input.wav'), smodel_vc, cmodel, net_g, hps, vc_utils)
            ...
            if state["step"] % 10000 == 0:
                loss = evaluate(val_dataloader,model,smodel, delta, criterion)
                with open(os.path.join(args.exp_dir, "record.txt"), "a") as f:
                    f.writelines([f"val iter {state['step']} delta {delta} loss: {loss},{noise[0,:].max()},{noise[0,:].min()}\n"])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--exp_dir", type=str, default="/mnt/sda/jinyu/attack_vc/exps/0220_blackbox512_MSE", help="path to pth file")
