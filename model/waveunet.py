...
class Waveunet(nn.Module):
    def __init__(self, num_inputs, num_channels, num_outputs, instruments, kernel_size, target_output_size, conv_type, res, separate=False, depth=1, strides=2):
        ...
        self.spk_conv = ConvLayer(1024, 1024, 2 , 1, conv_type)
    ...
    def forward_module(self, x, delta, src_emb, tar_emb, module):
        '''
        ...
        for conv in module.bottlenecks:
            # src_emb = self.src_linear(src_emb).unsqueeze(2)
            # tar_emb = self.src_linear(tar_emb).unsqueeze(2)
            emb = torch.cat((src_emb, tar_emb), 1)
            emb = self.linear(emb).unsqueeze(2)
            out = torch.cat((out, emb), 2)
            out = self.spk_conv(out)
            out = conv(out)
        ...
        # out = out.clamp(min=-1.0*delta, max=delta)
        return out

    def forward(self, x, delta, src_emb, tar_emb, inst=None):
        curr_input_size = x.shape[-1]
        assert(curr_input_size == self.input_size) # User promises to feed the proper input himself, to get the pre-calculated (NOT the originally desired) output size

        if self.separate:
            return {inst : self.forward_module(x, delta, src_emb, tar_emb, self.waveunets[inst])}
        else:
            assert(len(self.waveunets) == 1)
            out = self.forward_module(x, delta, src_emb, tar_emb, self.waveunets["ALL"])
